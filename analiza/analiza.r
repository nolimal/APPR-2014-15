...
cat("Rišem linearen graf Okunovega zakona za EU...\n")
...
text(2,23,paste0(round(lin$coefficients[1],2),round(lin$coefficients[2],2),"x"),cex=0.8)
detach(zanima)
dev.off()
#2.primer NELINEARNA ZVEZA
#Narišimo graf v PDF.
cat("Rišem kvadraten graf Okunovega zakona za EU...\n")
pdf("slike/EUOkunk.pdf")
attach(zanima)
plot(Rast,Nezaposleni)
kv <- lm(Nezaposleni ~ I(Rast^2) + Rast)
curve(predict(kv, data.frame(Rast=x)), add = TRUE, col = "red")
text(2,23,paste0(round(kv$coefficients[1],2),round(kv$coefficients[2],2),"x",round(kv$coefficients[3],2),"x^2"),cex=0.8)
detach(zanima)
dev.off()
...
#1.primer LINEARNA ZVEZA
cat("Rišem linearen graf Okunovega zakona za Slovenijo...\n")
pdf("slike/EUOkunsl.pdf")
attach(zanima1)
...
#3.primer NAPOVEDNI MODEL
cat("Rišem napovedni model Okunovega zakona za Slovenijo...\n")
pdf("slike/EUOkunsn.pdf")
attach(zanima1)
plot(RastBDP,Neaktivni)
library(mgcv)
mls <- loess(Neaktivni ~ RastBDP)
curve(predict(mls, data.frame(RastBDP=x)), add = TRUE, col = "magenta")
# mgam <- gam(Neaktivni ~ s(RastBDP))
# curve(predict(mgam, data.frame(BDP=x)), add = TRUE, col = "cyan")
detach(zanima1)
dev.off()